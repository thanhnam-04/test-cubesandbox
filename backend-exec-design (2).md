...
- **Existing deployment/configuration (§1–§2):** the platform supplies the
  ...
  `SANDBOX_API_BASE_URL` (with no code default), template ID, network policy,
  secret storage, timeouts, logging/tracing, and the feature flag.
...
configuration. It is the absolute base URL of the existing CubeSandbox
**cube-api lifecycle service**. Only the lifecycle routes in §2.1 are
resolved relative to this URL. The runtime execution and file routes use the
per-sandbox `domain` returned by provisioning and are resolved against the
envd runtime URL described in §2.2.
...
`domain` in the provision response (§2.1) is different: it is per-sandbox
...
Lifecycle and runtime authorization are platform-managed. Runtime access is
through the TLS/routing URL constructed from the provision response. The
deployment's network allowlist, TLS policy, and any additional envd
authentication must be confirmed with the platform developer before
implementation.

The **sandbox ID** is provisioned by cube-api and identifies which sandbox a
request targets.
...
combined with access to the runtime route is full access to that sandbox, so
the ID is treated as a credential even though it functions as an identifier.
...
The current CubeSandbox API is split across two services. `cube-api` owns
sandbox lifecycle. The sandbox's `envd` runtime owns command execution and
file transfer, reached through cube-proxy using the routing data returned by
cube-api. The implementation must not replace these with a simplified
single combined sandbox-execution endpoint unless the platform developer
confirms that the deployed platform actually exposes such an adapter.

### 2.1 Sandbox lifecycle through cube-api

The lifecycle base is `SANDBOX_API_BASE_URL`:

```
POST {SANDBOX_API_BASE_URL}/sandboxes
Headers:
  Content-Type: application/json
Body:
  { "templateID": "<template-id>" }
Response (2xx JSON):
  { "sandboxID": "<sandbox-id>", "domain": "<routing-domain>" }

DELETE {SANDBOX_API_BASE_URL}/sandboxes/{sandboxID}
Response:
  2xx; response body is not required by the current client
```

The backend must validate that provisioning returned both `sandboxID` and
`domain`, store both with the chat's sandbox record, and keep them backend-
only. A template ID is required by this current contract. Any lifecycle
authorization and related deployment configuration must use the platform's
existing security/configuration system rather than being invented in the
adapter.

After provisioning, the reference client initializes the two workspace roots
with an idempotent `exec` call: `mkdir -p /output /scratch`. The backend must
perform the equivalent initialization before exposing the sandbox to the
tool loop, or confirm that the platform image already guarantees those roots.

### 2.2 Command execution through envd's Connect-RPC endpoint

After provisioning, the current client constructs the envd runtime base URL
as follows:

```
ENVD_BASE_URL = https://{ENVD_PORT}-{sandboxID}.{domain}
ENVD_PORT     = 49983    # current CubeSandbox runtime port; confirm with platform
```

The command request is **not** JSON `{ command, cwd }` sent to cube-api. It is
a server-streaming Connect-RPC request:

```
POST {ENVD_BASE_URL}/process.Process/Start
Headers:
  Content-Type: application/connect+json
  Connect-Protocol-Version: 1
Body:
  Connect frame containing:
  {
    "process": {
      "cmd": "/bin/sh",
      "args": ["-c", "<command>"],
      "cwd": "<absolute-sandbox-working-directory>"
    }
  }
```

Each Connect frame is encoded as a one-byte flags field, a four-byte
big-endian payload length, and a JSON payload. The request uses a regular
message frame (`flags = 0x00`). The current client defaults `cwd` to
`/output`; the adapter maps the tool's relative working-directory convention
to an absolute sandbox path before constructing this request (`scratch/...`
maps to `/scratch/...`, other relative paths map under `/output`).

The HTTP response remains open for the command lifetime and contains a
sequence of Connect frames. Regular response frames carry events shaped like:

```json
{ "event": { "data": { "stdout": "<base64-bytes>" } } }
{ "event": { "data": { "stderr": "<base64-bytes>" } } }
{ "event": { "end": { "exitCode": 0 } } }
```

`stdout` and `stderr` payloads are base64-encoded independently and must be
decoded before being relayed to the LLM or UI. The `event.end.exitCode` value
is the command's exit code. The stream then terminates with a Connect
end-stream frame (`flags & 0x02`); an error in that trailer is a failed exec,
not a successful command with empty output.

The backend should relay each decoded data event live, accumulate the full
bounded result, and treat a stream that produces no frame for the configured
idle window as a terminal exec failure. The current reference client uses a
180-second idle window, reset whenever a response chunk arrives; it does not
impose a total command-duration limit.

### 2.3 File transfer through envd

File bytes use the same `ENVD_BASE_URL`, not a cube-api sandbox-file route:

```
GET {ENVD_BASE_URL}/files?path=<sandbox-path>
Response:
  2xx raw file bytes

POST {ENVD_BASE_URL}/files?path=<sandbox-path>
Body:
  multipart/form-data with one `file` part containing the file bytes
Response:
  2xx
```

The current client percent-encodes each path segment while preserving `/` as
the path separator. `write_file` uses this endpoint directly and must not
encode file contents into a shell command. File reads/writes are idempotent
in the adapter and may use the bounded file-transfer retry policy in §4;
general command execution must not be retried automatically.
...
3. **Required deployment configuration.** Confirm how the platform injects
   `SANDBOX_API_BASE_URL`, the cube-api template ID, platform-managed
   lifecycle/runtime authorization, and the current envd routing-port
   constant. The sandbox
   address must be environment-specific deployment configuration with no
   default or testing-server fallback. Confirm whether the platform's
   runtime route, TLS policy, or envd authentication differs from §2.2. If
   the platform does not provide these values (or provides a materially
   different contract), stop and re-ask the platform developer before
   implementation; do not invent an address or a parallel configuration
   surface.
